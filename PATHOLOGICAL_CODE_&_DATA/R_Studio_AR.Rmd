...
Data = read.csv("Pathological.csv", header=TRUE)
row.cnt = nrow(Data)
...
attach(Train)
head(Train)
write.csv(Test,"Test_Pathological.csv")
...
png(file="11 - Boxplot_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
...
png(file="12 - Boxplot_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
...
png(file="13 - Boxplot_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
...
png(file="14 - Boxplot_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
...
png(file="15 - Boxplot_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(2,2))
...
png(file="16 - Histogram_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
for (f in features[1:3]) {
...
png(file="17 - Histogram_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
for (f in features[4:6]) {
    hist(Train[,f],ylab="Frequency", xlab=str_to_title(f),main = "", col='orange', pch = 16,lwd = 2, lty =1)
...
png(file="18 - Histogram_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
for (f in features[7:9]) {
    hist(Train[,f],ylab="Frequency", xlab=str_to_title(f),main = "", col='orange', pch = 16,lwd = 2, lty =1)
...
png(file="19 - Histogram_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(1,3))
for (f in features[10:12]) {
    hist(Train[,f],ylab="Frequency", xlab=str_to_title(f),main = "", col='orange', pch = 16,lwd = 2, lty =1)
...
png(file="20 - Histogram_Pathological.png", res=300, width=1500, height=1500)
par(mfrow=c(2,2))
for (f in features[13:16]) {
    hist(Train[,f],ylab="Frequency", xlab=str_to_title(f),main = "", col='orange', pch = 16,lwd = 2, lty =1)
...
write.csv(results_model1,"21_Summary_Full_Model_Pathological.csv")
```
...
write.csv(anova(model1, test="Chisq"),"22_Anova_Pathological.csv")
```
...
```{r}
png(file="23 - Outlier_Full_Model_Pathological.png", res=300, width=2000, height=2000)
...
png(file="24 - Std_Resids_Full_Model_Pathological.png", res=300, width=2000, height=2000)
ggplot(model1.data, aes(index, .std.resid)) + 
  geom_point(aes(colour = Fetal.Health), alpha = 0.5) +
  theme_bw() + labs(x = "Observation Index") + labs(y = "Standard Residuals") +labs(color = "Fetal Health")
dev.off()
```
...
library(car)
vif(model1)
write.csv(vif(model1),"25 - Multi_Coll_Full_Model_Pathological.csv")
```
...
write.csv(results_model1,"26 - FB_Stepwise_Pathological.csv")
```
...
write.csv(df,"27 - Lasso_Pathological.csv")
...
df <- as.data.frame(as.matrix(a))
write.csv(df,"28 - Elastic_Net_Pathological.csv")
```
...
#                      type = "response")
# # Obtain classifications using a classification threshold of 0.5
# predClass.lasso = ifelse(pred.lasso > 0.3, 1, 0)
...
preds = data.frame(Fetal_Health = Test$Fetal.Health, predClass.full,
                   predClass.step,predClass.elnet)
preds
write.csv(preds,"29 - Prediction_0.7_Pathological.csv")
```
...
write.csv(a,"30 - PM_Full_Model_0.7_Pathological.csv")
...
write.csv(a,"31 - PM_Stepwise_Model_0.7_Pathological.csv")
...
# write.csv(a,"32 - PM_Lasso_Model_0.7_Pathological.csv")
...
write.csv(a,"33 - PM_Elastic_Model_0.7_Pathological.csv")
```
